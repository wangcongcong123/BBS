		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
	            	...
	            	html += '						<div class="filePicker">click to pick file</div>';
	            	html += '						<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
	            	html += '					</div>';
	            	html += '				</div>';
					html += '				<span id="fileDragArea" class="upload_drag_area">drag file onto over here</span>';
					html += '			</div>';
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">Files 0 selected, in total 0B。</div>';
		            html += '				<div class="btns">';
		            html += '					<div class="webuploader_pick">Pick Cont.</div>';
		            html += '					<div class="upload_btn">Start Upload</div>';
		            html += '				</div>';
		            html += '			</div>';
		            ...
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">0 file(s) selected，in total 0B。</div>';
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
		            html += '					<div class="webuploader_pick">pick file</div>';
		            html += '					<div class="upload_btn">upload</div>';
		            html += '				</div>';
		            html += '			</div>';
		            html += '			<div id="preview" class="upload_preview">';
				    html += '				<div class="add_upload">';
				    html += '					<a style="height:'+para.itemHeight+';width:'+para.itemWidth+';" title="click here to add files" id="rapidAddImg" class="add_imgBox" href="javascript:void(0)">';
					...
					html += '		</div>';
					html += '		<div class="upload_submit">';
					html += '			<button type="button" id="fileSubmit" class="upload_submit_btn">confirm to upload</button>';
					html += '		</div>';
					html += '		<div id="uploadInf" class="upload_inf"></div>';
					html += '	</div>';
					html += '</form>';
			...
			this.funSetStatusInfo = function(files){
				...
				// 设置内容
				$("#status_info").html("select "+num+" file, size in total "+size+"。");
			};
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					if (file.size >= 51200000) {
						alert('your file "'+ file.name +'"the size is too big');	
					} else {
						// 在这里需要判断当前所有文件中
						arrFiles.push(file);	
					}
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '	<p id="uploadFailure_'+file.index+'" class="file_failure">failed, try again later</p>';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						$("#uploadInf").append("<p>successfully, save to path：" + response + "</p>");
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
					...
					onFailure: function(file) {
						...
						$("#uploadSuccess_" + file.index).show();
						alert("failed");
						$("#uploadInf").append("<p>file:" + file.name + "failed！</p>");	
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					...
					if(ZYFILE.funReturnNeedFiles().length > 0){
						...
						alert("pick at least a file first, please");
					}
	            });
