...
var rawUniversitiesData=new Array();

for (var i=0;i<schoolList.length;i+=1){
    var city={};
    var schoolsInCity=schoolList[i].school;
    city.name=schoolList[i].name;

    city.sub=[];
    city.sub.push({name: "请选择", sub: []});
    
    for (var j=0;j<schoolsInCity.length;j+=1){
        var school={};
        
        school.name=schoolsInCity[j].name;
        school.sub=[];
        school.type=0;

        city.sub.push(school);
    }
    city.sub.push({name: "其他"});

    city.type=1;
    rawUniversitiesData.push(city);
    
}


// console.log(rawUniversitiesData);
